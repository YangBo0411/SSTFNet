...
class Trainer:
    ...
    #调用 before_train: 进行训练前的初始化工作，如模型、优化器、数据加载器的设置。
    # 执行训练循环 train_in_epoch: 进行具体的训练过程。
    # 异常处理: 捕获任何在训练过程中发生的异常，并重新抛出。
    # 调用 after_train: 进行训练后的清理和日志记录工作。
    def train(self):
        self.before_train()
        try:
            self.train_in_epoch()
    ...
    # 遍历所有轮次: 从 self.start_epoch 到 self.max_epoch 进行循环。
    # 调用 before_epoch: 进行每个轮次开始前的准备工作，如数据增强设置。
    # 调用 train_in_iter: 进行该轮次内的所有训练迭代。
    # 调用 after_epoch: 进行每个轮次结束后的工作，如保存模型和评估
    ...
    # 遍历所有迭代: 从 0 到 self.max_iter 进行循环。
    # 调用 before_iter: 进行每个迭代开始前的准备工作（当前代码中为空）。
    # 调用 train_one_iter: 执行单个训练迭代，包括前向传播、反向传播和优化步骤。
    # 调用 after_iter: 进行每个迭代结束后的工作，如日志记录和学习率调整。
    ...
    # 执行单个训练迭代，包括数据加载、前向传播、损失计算、反向传播、优化步骤、学习率更新和指标记录
    def train_one_iter(self):
        iter_start_time = time.time()

        inps, targets,_ = self.prefetcher.next()        # inps 16，3，576，576  targets 16 120 5
    ...
    #训练开始前进行一系列初始化工作，包括模型、优化器、数据加载器、学习率调度器、分布式训练设置、模型 EMA、评估器和日志记录器的初始化
