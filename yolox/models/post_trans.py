...
class Attention_msa(nn.Module):
    # 输入原始特征信息 B N C 1 480 124 
    # 输出经多头注意力处理后的特征与原始特征拼接之后的特征，以及注意力权重
    ...
    def forward(self, x_cls, x_reg, cls_score=None, fg_score=None,   # x_cls 1 480 128  cls_score 480
                return_attention=False, ave=True, sim_thresh=0.75,
                use_mask=False,**kwargs):
        B, N, C = x_cls.shape           # B 1 N 480 C 128

        qkv_cls = self.qkv_cls(x_cls).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)    # 3 1 4 480 32
        qkv_reg = self.qkv_reg(x_reg).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)    # 3 1 4 480 32
        q_cls, k_cls, v_cls = qkv_cls[0], qkv_cls[1], qkv_cls[2]  # make torchscript happy (cannot use tensor as tuple) q k v  1 4 480 32
        q_reg, k_reg, v_reg = qkv_reg[0], qkv_reg[1], qkv_reg[2]  # 第0维度分离Q、K、V 1 4 480 32 

        q_cls = q_cls / torch.norm(q_cls, dim=-1, keepdim=True)  # 通过对每个向量在通道维度进行L2归一化，确保数值稳定性 1 4 480 32 
        ...
        if cls_score == None:
            ...
            # 将分类得分cls_score重新调整形状并扩展，以便与多头自注意力机制中的注意力头数量和特征序列长度相匹配
            # 将 cls_score 从原始形状调整为 [1, 1, 1, -1]，然后重复（repeat）这个张量以适应多个注意力头和特征序列长度
            cls_score = torch.reshape(cls_score, [1, 1, 1, -1]).repeat(1, self.num_heads, N, 1)   # 1 4 480 480
        ...
        attn_cls_raw = v_cls_normed @ v_cls_normed.transpose(-2, -1)        # 将 v_cls_normed 与它转置后的结果进行矩阵乘法 自注意力机制
        ...
        attn_cls = (q_cls @ k_cls.transpose(-2, -1)) * self.scale * cls_score * cls_score_mask  #  1 4 480 480  计算 q_cls 与 k_cls 的矩阵乘法，得到形状 [B, num_heads, N, N]
        ...
        # attn_reg = (q_reg @ k_reg.transpose(-2, -1)) * self.scale  * fg_score * fg_score_mask         # yb
        ...
        attn_cls = attn_cls.softmax(dim=-1)             # 在最后一个维度上归一化  1 4 480 480
        attn_cls = self.attn_drop(attn_cls)             # 随机丢弃部分注意力权重
        ...
        # attn = attn_reg                           # yb
        x = (attn @ v_cls).transpose(1, 2).reshape(B, N, C)  # 注意力权重 attn 与 v_cls 进行矩阵乘法，在进行维度变换 1 480 128
        # x = (attn_cls @ v_cls).transpose(1, 2).reshape(B, N, C)  # 注意力权重 attn 与 v_cls 进行矩阵乘法，在进行维度变换 1 480 128  # yb
        x_ori = v_cls.permute(0, 2, 1, 3).reshape(B, N, C)   # 保存原始 v_cls 的输出 1 480 128
        x_cls = torch.cat([x, x_ori], dim=-1)       # 将注意力输出 x 与原始 v_cls 输出 x_ori 拼接，在最后一个维度上连接，得到形状 [B, N, 2 * C] 1 480 256
        #
        x_reg = (attn @ v_reg).transpose(1, 2).reshape(B, N, C)
        # x_reg = (attn_reg @ v_reg).transpose(1, 2).reshape(B, N, C)      # yb
        x_ori_reg = v_reg.permute(0, 2, 1, 3).reshape(B, N, C)
        x_reg = torch.cat([x_reg, x_ori_reg], dim=-1)

        if ave:
            conf_sim_thresh = kwargs.get('conf_sim_thresh', 0.99)    # 0.99
            ones_matrix = torch.ones(attn.shape[2:]).to('cuda')   # 创建全1矩阵 ones_matrix 和全0矩阵 zero_matrix，形状为 [N, N]
            zero_matrix = torch.zeros(attn.shape[2:]).to('cuda')

            attn_cls_raw = torch.sum(attn_cls_raw, dim=1, keepdim=False)[0] / self.num_heads # 480 480 对 attn_cls_raw 在 num_heads 维度上求和，并除以 self.num_heads，得到平均的注意力权重，形状为 [B, N, N]。这里 [0] 选择批次维度的第一个元素
            attn_reg_raw = torch.sum(attn_reg_raw, dim=1, keepdim=False)[0] / self.num_heads
            sim_mask = torch.where(attn_cls_raw > sim_thresh, ones_matrix, zero_matrix) #相似度掩码 对比 attn_cls_raw 是否大于 sim_thresh，如果是，则对应位置为1，否则为0
            #remove ave and conf guide in the reg branch, modified in 2023.12.5
            ...
            sim_attn = torch.sum(attn, dim=1, keepdim=False)[0] / self.num_heads  # 480 480 attn 在 num_heads 维度上求和，并除以 self.num_heads，得到平均的注意力权重， [0] 选择批次维度的第一个元素
            sim_round2 = torch.softmax(sim_attn, dim=-1)   #480 480 对 sim_attn 应用 softmax，得到归一化的相似度权重 sim_round2
            sim_round2 = sim_mask * sim_round2 / (torch.sum(sim_mask * sim_round2, dim=-1, keepdim=True)) # sim_round2 与 sim_mask 相乘，确保只有满足相似度阈值的部分被保留，按行（最后一个维度）进行归一化，确保每一行的权重和为1
            obj_mask = obj_mask * sim_round2 / (torch.sum(obj_mask * sim_round2, dim=-1, keepdim=True))
           
            ##-----------------yb---------------------------
            # sim_attn = torch.sum(attn_cls, dim=1, keepdim=False)[0] / self.num_heads  # 480 480 attn 在 num_heads 维度上求和，并除以 self.num_heads，得到平均的注意力权重， [0] 选择批次维度的第一个元素
            # sim_round2 = torch.softmax(sim_attn, dim=-1)   #480 480 对 sim_attn 应用 softmax，得到归一化的相似度权重 sim_round2
            # sim_round2 = sim_mask * sim_round2 / (torch.sum(sim_mask * sim_round2, dim=-1, keepdim=True)) # sim_round2 与 sim_mask 相乘，确保只有满足相似度阈值的部分被保留，按行（最后一个维度）进行归一化，确保每一行的权重和为1
            # obj_attn = torch.sum(attn_reg, dim=1, keepdim=False)[0] / self.num_heads
            # obj_mask = obj_mask * obj_attn / (torch.sum(obj_mask * obj_attn, dim=-1, keepdim=True))
            ##-----------------yb---------------------------
            return x_cls, x_reg, sim_round2, obj_mask  # sim_round2 类别自注意机制与（类别、目标注意力）整合之后的综合注意力权重
        else:                                          # obj_mask 目标自注意机制与（类别、目标注意力）整合之后的综合注意力权重
...
class MSA_yolov(nn.Module):
    ...
    def find_similar_round2(self, features, ave_mask,feature_obj,mask_obj,fg_score=None):
        key_feature = features[0]       # 480 256  16*30 256  bs=16时，每张图片挑选的30个特征互为关键帧和支持帧
        support_feature = features[0]
        if not self.training:
            ave_mask = ave_mask.to(features.dtype)
        soft_sim_feature = (ave_mask @ support_feature)    # 480 256 利用注意力机制从支持帧中找到相似特征
        cls_feature = torch.cat([soft_sim_feature, key_feature], dim=-1)  # 480 512 整合关键帧与相似帧的特征
        if self.reconf:
    ...
    def forward(self, x_cls, x_reg, cls_score=None, fg_score=None, sim_thresh=0.75, ave=True, use_mask=False,**kwargs):
        # 输入原始特征信息 B N C 1 480 124 
        # trans_cls, trans_obj 输出经多头注意力处理后的特征与原始特征拼接之后的特征
        # ave_mask,obj_mask 类别（目标）自注意机制与（类别、目标注意力）整合之后的综合注意力权重
        ...
        trans_cls,trans_obj = self.find_similar_round2(trans_cls, ave_mask, trans_obj, obj_mask) # 对支持帧利用注意力机制，找到与关键帧相似的支持帧特征，融合关键帧与相似帧的特征，只对类别进行操作
