...
class VIDDataset(torchDataset):
    ...
    def photo_to_sequence(self,dataset_path,lframe,gframe):
        ...
        for element in dataset:                             # 遍历每个视频序列
            ele_len = len(element)                          # 当前视频序列的长度
            if ele_len<lframe+gframe:
                ...
                if self.mode == 'random':               # 随机模式 随机打乱当前视频序列的帧顺序，按gframe切割并添加到结果中
                    if lframe == 0:
                ...
                # 新增的连续采样模式
                                    # 修改后的连续采样模式（非重叠）
                    split_num = ele_len // gframe  # 计算可以分割的完整片段数量
                    for i in range(split_num):
                        start_idx = i * gframe
                        continuous_frames = element[start_idx:start_idx + gframe]
                        res.append(continuous_frames)
                    # 处理剩余帧（可选）
                    remaining = ele_len % gframe
                    if self.formal and remaining != 0:
                        tail = element[-remaining:]
                        res.append(tail)
                # #----  yb   -----
    ...
    def get_annotation(self,path,test_size):                                # 获得xml标签中的信息
        path = path.replace("Data","Annotations").replace("png","xml")           #yb ITSDT  bmp
        ...
        anno_res = []
        for xmls in files:
            photoname = xmls.replace("Annotations","Data").replace("xml","png")    #yb  ITSDT  bmp
    ...
    def pull_item(self,path):                   #获取单个图像及其标注信息的方法
        """
